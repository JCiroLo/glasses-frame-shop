...
import '../assets/css/TestGlasses.scss'
...
const getPhotoDimensions = dataURL =>
  new Promise(resolve => {
...
const getResponsiveDimensions = ({ width, height }, navbarHeight) => {
  ...
  const stageWidth = onMobile ? windowWidth : windowHeight * ratio
  const stageHeight = onMobile ? windowWidth * (1 / ratio) : windowHeight
...
const TestGlasses = ({ navbarHeight = 75, history }) => {
  ...
  const [selectedMount, setSelectedMount] = useState(
    history.location.state !== null
      ? `https://firebasestorage.googleapis.com/v0/b/optica-809a9.appspot.com/o/${history.location.state.frame}_testing.png?alt=media&token=2cc6b58c-a5fe-4688-83eb-8f0337e2e7cc`
      : null
  )
  ...
  const checkDeselect = e => {
    const clickedOnEmpty =
      e.target === e.target.getStage() || e.target.attrs.id === '1000'
    if (clickedOnEmpty) {
      selectShape(null)
    }
  }
  ...
  const HandleUsePhoto = async e => {
    ...
    const trueDimensions = getResponsiveDimensions(
      { width, height },
      navbarHeight
    )
  ...
  const HandleUseTestPic = async () => {
    const url =
      'https://image.freepik.com/free-photo/portrait-young-beautiful-woman-with-fresh-clean-skin-isolated-white_186202-1721.jpg'
    const { width, height } = await getPhotoDimensions(url)
        ...
        <>
          ...
          <div className='test-actions-container'>
            <div className='test-actions'>
              ...
              <span className='material-icons btn-camera'>camera_alt</span>
              <span
                className='material-icons btn-search'
                onClick={() => {
                  setShowMounts(true)
      ...
      {cameraState === 0 && (
        <>
          <div className='glitch-wrapper row mx-0'>
            ...
            <div className='glitch' data-text='Error :('>
              Error :(
            </div>
