...
import '../assets/css/Management.scss'
...
const Management: FC<ManagementProps> = ({ location }) => {
  ...
  // Create glasses
  const [createGlassesInfo, setCreateGlassesInfo] = useState<GlassFrameType>({
    _id: '',
    brand: '',
    color: 'no',
    shape: 'no',
  ...
  const [selectedFrame, setSelectedFrame] = useState<GlassFrameType>({
    _id: '',
    brand: '',
    color: 'no',
    shape: 'no',
  ...
  const [allGlasses, setAllGlasses] = useState<GlassFrameType[]>([])
  // Modals
  ...
  const emptyFrame: GlassFrameType = {
    _id: '',
    brand: '',
    color: 'no',
    shape: 'no',
      ...
      if (!sessionStorage.token) {
        ...
        if (validate) {
          ...
          if (status) {
            ...
            setError('Error: Cargar monturas.')
          }
  ...
  const validateForm = (
    ...
    if (action === 'create') {
      const { brand, price, color, material, shape, description, discount } =
        createGlassesInfo
  ...
  const openModal = (type: string, obj?: GlassFrameType) => {
    if (type === 'create') {
  ...
  const handleForm = (e: ChangeEvent<HTMLInputElement>, action: string) => {
    ...
    if (action === 'create')
      ...
      setSelectedFrame({ ...selectedFrame, [id]: value })
  }
  ...
  const onDelete = async (currentGlasses: GlassFrameType) => {
    setLoading(true)
  ...
  const formSection = (action: string, frame: GlassFrameType): JSX.Element => (
        ...
        <div className='col-12 col-md-6 col-lg-3'>
          <label htmlFor='images' className='upload-images'>
            ...
            <img
              src={UploadImg}
              alt=''
              style={{ visibility: thumbs.length > 0 ? 'hidden' : 'visible' }}
            />
            <input
              type='file'
              id='images'
      ...
      <div className='modals secondary-font'>
        ...
        {stateModals.delete && (
          <Modal
            Title={
              <div>
                ¿Estas seguro que deseas eliminar "{selectedFrame.brand}" ?
              </div>
            }
            ...
            Actions={
              <>
                <button
                  className='confirm'
                  onClick={() => onDelete(selectedFrame)}
                >
                  Eliminar
                </button>
                <button className='cancel' onClick={closeModal}>
              ...
              <div className='uploading-progress secondary-font'>
                <div className='progress'>
                  <div
                    ...
                    role='progressbar'
                    aria-valuenow={
                      uploadProgress.model / 2 + uploadProgress.testing / 2
                    }
                    aria-valuemin={0}
                    aria-valuemax={100}
                    style={{
        ...
        <>
          ...
          <div className='manage-actions secondary-font'>
            <button className='btn' onClick={() => openModal('create')}>
              <span className='material-icons'>add</span> Agregar montura
            </button>
            <button className='btn'>
              <span className='material-icons'>insert_link</span> Otra acción
            </button>
          </div>
            ...
            {allGlasses.map((frame, index) => (
              ...
              >
                <span
                  ...
                  onClick={() => openModal('update', frame)}
                >
                  edit
                </span>
                <span
                  className='material-icons'
                  onClick={() => openModal('delete', frame)}
                >
                  delete
                </span>
