...
import '../assets/css/DetailMount.scss'
      ...
      if (id !== undefined) {
        ...
        if (stateAction) {
          ...
          setError({
            status: true,
            message:
              'El sitio al que estás intentando acceder no está disponible o ha sido eliminado.'
          })
      ...
      {!loading && !error.status && (
        <div className='secondary-font row mx-0'>
          ...
          <div className='detail-content col-12 col-md-6'>
            <h3 className='text-uppercase font-weight-bold principal-font'>
              {mountDetail.brand}
            </h3>
            <h4 className='font-weight-bold'>
              Forma:{' '}
              <span className='font-weight-normal'>
                {formatText(mountDetail.shape)}
              </span>
            </h4>
            <h4 className='font-weight-bold'>
              Material:{' '}
              <span className='font-weight-normal'>
                {formatText(mountDetail.material)}
              </span>
            </h4>
            <h4 className='font-weight-bold '>
              Color:{' '}
            ...
            <h4 className='font-weight-bold'>
              Precio:{' '}
              <span className='font-weight-normal h2 text-premium with-line'>
                  ...
                  <>
                    ...
                    <strong>
                      ...
                      {new Intl.NumberFormat('de-DE').format(
                        mountDetail.price * (1 - mountDetail.discount / 100)
                      )}
                    </strong>
                  </>
                ) : (
            ...
            <div className='actions'>
              <button
                className='btn btn-shop my-4'
                onClick={() => alert('Comprando')}
              >
                <span className='material-icons'>shop</span> Comprar ahora
              </button>

              <div className='text-premium' onClick={testFrame}>
                <span className='material-icons'>face</span>{' '}
                <span>Probar montura</span>
              </div>
